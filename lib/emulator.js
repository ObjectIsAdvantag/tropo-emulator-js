...
var optionDefinitions = [
    ...
    { name: 'trequest', alias: 't', type: Boolean }                     // dumps the Tropo request library
];
...
function help() {
    ...
    console.log("   -t,--trequest                 : dumps the HTTP client library to inject on top of your Tropo script");
...
if (args.trequest) {
    /* TODO: turn to the possibility to load file into a string
    var fs = require('fs');
    var contents = fs.readFileSync('../lib/trequest.js', 'utf8');
    */
    var contents = tropo_request.toString();
...
function trequest(options, cb) {
    ...
    // Wraps the request function to handle Tropo specifics (error messages...)
    request(options, function (err, response, body) {
        if (err) {
            if (err.message === "ESOCKETTIMEDOUT") { // Connect timeout
                err.message = "java.net.SocketTimeoutException: Connect timed out";
            }
            if (err.message === "ETIMEDOUT") { // Read timeout
                err.message = "java.net.SocketTimeoutException: Read timed out";
            }
            if (err.message.match(/^getaddrinfo ENOTFOUND/)) { // No connectivity 
                // TODO: identify Tropo behavior if server cannot be reached ?
                err.message = "java.net.SocketTimeoutException: Connect timed out";
            }
